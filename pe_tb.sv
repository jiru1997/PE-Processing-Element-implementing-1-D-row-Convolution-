...
//pe testbench
module pe_tb(interface filter_in, filter_addr, ifmap_in, ifmap_addr, psum_in, start, done, psum_out);
...
//DUT (pe)
 pe #(.WIDTH(WIDTH), .DEPTH_I(DEPTH_I), .ADDR_I(ADDR_I), .DEPTH_F(DEPTH_F), .ADDR_F(ADDR_F)) 
